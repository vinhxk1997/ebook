$(document).ready(function () {
    ...
    $(document).on('click', '.on-story-preview', function () {
        ...
        if ($(this).hasClass('unvoted')) {
            ...
            $.ajax({
                url: ebook.base_url + '/chapter/' + $(this).data('id') + '/vote',
                method: 'post',
                cache: false,
            });
        } else {
            $('.button-vote').find('button').removeClass('voted');
            $('.button-vote').find('button').addClass('unvoted');
            $.ajax({
                url: ebook.base_url + '/chapter/' + $(this).data('id') + '/unvote',
                method: 'post',
                cache: false,
                data: {
                    _method: 'delete'
